<style>
    #wrapper {width:960px;margin:auto;}
    #main-slider {margin:30px auto;height:400px;position:relative;background:url(/skins/img/shadow.png) right 160px no-repeat;}
    #internal-slider{width:930px;margin:auto;}
    #social-buttons{position:absolute;right:40px;top:30px;}
    #social-buttons a{margin-left:12px;display:block;float:left;}
    #select{position:absolute;right:0px;top:172px;}
    #prev{display:block;width:61px;height:58px;background:url(/skins/img/prev.png)no-repeat;float:left;}
    #next{display:block;width:56px;height:58px;background:url(/skins/img/next.png)no-repeat;margin-left:60px;}
    #bottom-select{position:absolute;left:45%;top:100%;}
    #left-dot, #center-dot, #right-dot{display:block;width:18px;height:17px;float:left;margin-left:15px; }
    #left-dot{background:url(/skins/img/left-dot.png)no-repeat;}
    #center-dot{background:url(/skins/img/center-dot.png)no-repeat;}
    #right-dot{background:url(/skins/img/center-dot.png)no-repeat;}
    .top-content{margin-top:50px;overflow:hidden;}
    .left-content{padding-left:25px;float:left;width:700px;}
    .right-content{margin:20px 0 0 700px;}
    .left-content h2{color:#f93c18;font-size:35px;line-height:1.2;font-weight:600;margin:0;}
    .left-content p{font-size:21px;line-height:1.5;color:#3a4e70;margin-bottom:0; }
    .left-content a {display:block;background:#2483cb;width:99px;text-decoration:none;color:#fff;
    font-size:18px;padding:8px 30px;border-radius:3px;float:right;margin-right:30px;}
    .middle-content{margin-top:50px;overflow:hidden;}
    .left-middle{float:left;padding-left:40px;}
    .center-middle{margin-left:360px;}
    .right-middle{float:right;padding-top:30px;}
    .left-middle span{display:block;color:#b8bec7;margin-bottom:10px;}
    .left-middle p{display:inline;color:#1fbba6;font-size:35px;margin-left:15px;}
    .right-middle a{display:block;background:#e86950;text-decoration:none;color:#fff;font-size:25px;
    padding:13px 30px;border-radius:3px;}
    .bottom-content{margin:50px 150px 40px 80px;}
    .bottom-content p{color:#384c6e;font-size:12px;padding:}
    .bottom-content a{color:#68bca7;}
</style>



<div id ="wrapper">
        <div id ="main-slider">
            <div id ="internal-slider">
                <div id="social-buttons">
                    <a href="#"><img src="/skins/img/youtube.png" alt="youtube"/></a>
                    <a href="#"><img src="/skins/img/google-plus.png" alt="google-plus"/></a>
                    <a href="#"><img src="/skins/img/skype.png" alt="skype"/></a>
                    <a href="#"><img src="/skins/img/vk.png" alt="vkontakte"/></a>
                    <a href="#"><img src="/skins/img/fb.png" alt="facebook"/></a>
                    <a href="#"><img src="/skins/img/tw.png" alt="twitter"/></a>
                </div>
                <div id="select">
                    <a href="#" id="prev"></a>
                    <a href="#" id="next"></a>
                </div>
                <div id="bottom-select">
                    <a href="#" id="left-dot"></a>
                    <a href="#" id="center-dot"></a>
                    <a href="#" id="right-dot"></a>
                </div>
                    <img src="/skins/img/slider.jpg" alt="slider"/>
            </div>
        </div>
        <div class="top-content">
            <div class="left-content">
                <h2>Максимальный призовой фонд<br> за минимальную стоимость участия</h2>
                <p>Текст о том, как проводятся турниры по легендарным играм, какая стоимость участия
                и правила турниров. Текст о том, как проводятся турниры по легендарным играм, какая стоимость участия
                    и правила турниров.</p>
                <a href="#">Подробнее</a>

            </div>
            <div class="right-content">
                <img src="/skins/img/sticker.jpg" alt="Только лучшее"/>
            </div>
        </div>
        <div class="middle-content">
            <div class="left-middle">
                <span>Участников</span>
                <img src="/skins/img/inquiries.png"/>
                <p>+350</p>
            </div>
            <div class="right-middle">
                <a href="#">Принять участие</a>
            </div>
            <div class="center-middle">
                <img src="/skins/img/payment.jpg"/>
            </div>
            <div class="bottom-content">
                <p>Все условия и правила ближайшего турнира читайте на нашем сайте <a href="/tournament">http://gs11.ru/tournament</a>.
                Участие в турнирах возможно только в том случае, если вы зарегистрировались на сайте <a href="/">GS11</a>. Это дает вам возможность
                не только участвовать в турнирах, но и вести блоги по пройденным играм, читать новости всей игровой индустрии и многое другое... </p>
            </div>


        </div>
</div>